...
class ChatAppConversationView extends StatefulWidget {
  ...
  @override
  State<ChatAppConversationView> createState() => _ChatAppConversationViewState();
}
...
class _ChatAppConversationViewState extends State<ChatAppConversationView> {
  ...
  void initState() {
    super.initState();
    _controller.text = 'Hello?! this is a message! If you read it for long enough, '
...
class TimestampedChatMessageRenderObject extends RenderBox {
  ...
  Size _layoutText(double maxWidth) {
    ...
    if (textLines.length == 1) {
      ...
      _sentAtFitsOnLastLine = lastLineWithDate < min(_longestLineWidth, maxWidth);
    }
